...
class NewsSignal(BaseModel):
    btc_signal: Literal[-1, 0, 1] = Field(
        description="""
            The signal of the news on the BTC price.
            1 if the price is expected to go up
            -1 if it is expected to go down.
            0 if the news is not related to the coin.

            If the news is not related to the BTC, then singal should be 0.
        """
    )
    eth_signal: Literal[-1, 0, 1] = Field(
        description="""
            The signal of the news on the ETH price.
            1 if the price is expected to go up
            -1 if it is expected to go down.
            0 if the news is not related to the coin.

            If the news is not related to the ETH, then singal should be 0.
        """
    )
    reasoning: str = Field(
        description="""
                    The reasoning behind the signal based on the news.
                    """
    )


class ClaudeNewsSignalExtractor:
    def __init__(self, model_name: str, api_key: str, temperature: Optional[float] = 0):
